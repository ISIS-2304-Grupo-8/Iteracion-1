...
public class ClientesController {
    ...
    // clienteRepository.insertarCliente(cliente.getNum_doc(), cliente.getNombre(),
    // cliente.getEmail(), cliente.getTipo_doc(),
    // cliente.getRol_cliente().getId_usuario());
    ...
    public String clienteEditarGuardar(@PathVariable("num_doc") int num_doc, @ModelAttribute ClienteEntity cliente) {
        clienteRepository.actualizarCliente(num_doc, cliente.getNombre(), cliente.getEmail(), cliente.getTipo_doc(),
                cliente.getRol_cliente().getId_usuario());
        return "redirect:/clientes";
    }
    ...
    public ResponseEntity<Object> clienteDetalle(@PathVariable("num_doc") int num_doc) {
        ...
        if (cliente != null) {
            ...
            return new ResponseEntity<>(Collections.singletonMap("mensaje", "Cliente no encontrado"),
                    HttpStatus.NOT_FOUND);
        }
    }

    @GetMapping("/clientes/buenos_clientes")
    public ResponseEntity<Collection<ResponseGoodClients>> buenosClientes(@RequestParam(required = false) Integer size,
            @RequestParam(required = false) Integer offset) {
        Collection<ResponseGoodClients> buenosClientesPage = clienteRepository.darBuenosClientes(size, offset);
