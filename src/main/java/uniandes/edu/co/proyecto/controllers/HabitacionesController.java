...
public class HabitacionesController {
    ...
    public String habitacionGuardar(@ModelAttribute HabitacionEntity habitacion) {
        habitacionRepository.insertarHabitacion(habitacion.getConsumo_acumulado(), habitacion.getDisponibilidad(),
                habitacion.getTipos_habitacion_id_tipo().getId_tipo());
        return "redirect:/habitaciones";
    }
    ...
    public String habitacionEditarGuardar(@PathVariable("id_habitacion") Integer id_habitacion,
            @ModelAttribute HabitacionEntity habitacion) {
        habitacionRepository.actualizarHabitacion(id_habitacion, habitacion.getConsumo_acumulado(),
                habitacion.getDisponibilidad(), habitacion.getTipos_habitacion_id_tipo().getId_tipo());
        return "redirect:/habitaciones";
    }
    ...
    // Collection<RespuestaReq1> informacion =
    // habitacionRepository.darDineroRecolectadoServiciosPorHabitacion();
    // model.addAttribute("habitaciones",
    // informacion.iterator().next().getID_HABITACION());
    // model.addAttribute("dinero_recolectado",
    // informacion.iterator().next().getDINERO_RECOLECTADO());
    ...
    public ResponseEntity<Collection<RespuestaOcupacion>> obtenerIndiceOcupacion(
            @RequestParam(required = false) Integer size, @RequestParam(required = false) Integer offset) {
        Collection<RespuestaOcupacion> resultado = habitacionRepository.obtenerIndiceOcupacionPorHabitacion(size,
                offset);
